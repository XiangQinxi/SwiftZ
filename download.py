...
st.title("文件获取")
st.caption("输入查询 ID 与密码后，可查看压缩包中的原始文件并下载。")

with st.container(border=True):
    st.markdown(
        """
### 下载说明
- 输入正确的查询 ID 与密码后，系统会读取加密 ZIP
- 你可以逐个下载里面的原始文件
- 也可以一键下载全部文件（ZIP）
        """
    )
...
with st.form("download_form"):
    ...
    password = st.text_input("提取密码", type="password")
...
if submitted:
    ...
    if not package_id:
        ...
        try:
            package_info = api.get_package_info(package_id)
