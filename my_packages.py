...
if not packages:
    ...
    for package in packages:
        ...
        with st.container(border=True):
            ...
            meta_col.caption(f"{package.get('file_count', 0)} 个文件 · {'有密码' if package.get('encrypted', True) else '无密码'}")
            st.write(package.get("description") or "暂无描述")
            st.caption(f"上传时间：{package.get('created_at', '未知')} · {'已公开分享' if package.get('share') else '未公开'}")

            with st.expander("编辑信息"):
                with st.form(f"edit_{package_id}"):
                    description = st.text_area("文件描述", value=package.get("description") or "")
            ...
            if action1.button("前往获取页面", key=f"go_{package_id}", use_container_width=True):
                st.query_params["name"] = package_id
                st.switch_page("download.py")
            if action2.button("删除文件包", key=f"delete_{package_id}", use_container_width=True, type="secondary"):
                try:
